...
from modeling_wrapper import WRAPPERS, add_wrapper_config
from modeling_vadapter import ADAPTERS, add_adapter_config
...
class RobertaSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.recon_pair = None

        #adapter
        #if config.select_adapter is not None:
        #    self.adapter = ADAPTERS[config.select_adapter](config)
        #    self.adapter_flag = True

    def forward(self, hidden_states, input_tensor):
        hidden_states_p = self.dense(hidden_states)
        self.recon_pair = (hidden_states, hidden_states_p)
        hidden_states = self.dropout(hidden_states_p)
...
class RobertaOutput(nn.Module):
    def __init__(self, config, _):
        ...
        #adapter
        # if config.select_adapter is not None and _ >= config.num_hidden_layers-3:
        #     self.adapter = ADAPTERS[config.select_adapter](config)
        #     self.adapter_flag = True
        #     print('++++++', _)
        if config.select_adapter is not None:
            # if _ == 6 or _ == 8 or _ == 11:
            if _ in config.layer_sets:
                self.adapter = ADAPTERS[config.select_adapter](config, _)
                self.adapter_flag = True
                print('++++++', _)
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        if self.adapter_flag:
            hidden_states_p = self.adapter(hidden_states)
            self.recon_pair = (hidden_states, hidden_states_p)
            hidden_states = self.dropout(hidden_states_p)
...
class RobertaLayer(nn.Module):
    def __init__(self, config, _):
        ...
        self.output = RobertaOutput(config, _)
        self.recon_pair = None
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        self.recon_pair = self.output.recon_pair
        return layer_output


class RobertaEncoderDetached(nn.Module):

    def __init__(self, config):
        super().__init__()
        self.config = config
        self.wrapper_config = config.wrapper_config
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if output_attentions:
                if (self.finetune_layers is None) or (i+1 not in self.finetune_layers):
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaLayer(config, _) for _ in range(config.num_hidden_layers)])
        
        self.recon_pair_list = None
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],)
            recon_pair_list.append(layer_module.recon_pair)

        self.recon_pair_list = recon_pair_list
...
class RobertaModelOrig(RobertaPreTrainedModel):
    """
...
class RobertaModelWithWrapper(RobertaModelOrig):

    authorized_missing_keys = [r"position_ids"]

    # Copied from transformers.modeling_bert.BertModel.__init__ with Bert->Roberta
    def __init__(self, config, add_pooling_layer=True):
        config = add_wrapper_config(config)
        config = add_adapter_config(config)
        ...
        if config.select_wrapper is None:
            ...
            if config.select_adapter is None:
                self.encoder = RobertaEncoderDetached(config)
            self.wrapper = bert_wrapper_opt[config.select_wrapper](config)
            print('*' * 20)
            print('\n\nusing BertWithWrapper: %s\n\n' % config.select_wrapper)
            print('*' * 20)

            if config.wrapper_config is not None:
                print('*' * 20)
                print('\nWrapper config: %s\n' % str(config.wrapper_config))
                print('*' * 20)
            else:
                print('*' * 20)
                print('\nWrapper config: using default config') 
                print('*' * 20)
        ...
        if config.select_adapter is None:
            ...
            if config.select_wrapper is None:
                self.wrapper = bert_wrapper_opt['last_layer'](config)
            print('*' * 20)
            print('\n\nusing Adapter: %s\n\n' % config.select_adapter)
            print('*' * 20)
    ...
    def forward(
        ...
        self.recon_pair_list = self.encoder.recon_pair_list
...
RobertaModel = RobertaModelWithWrapper
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.recon_loss_list = None
        self.if_adapter = config.select_adapter
        self.init_weights()
    ...
    def forward(
        ...
        if self.if_adapter is not None:
            recon_pair_list = self.roberta.recon_pair_list
            recon_criterion = MSELoss()
            self.recon_loss_list = []
            for x in recon_pair_list:
                try:
                    self.recon_loss_list.append(0.001*recon_criterion(x[0], x[1]))
                except:
                    self.recon_loss_list.append(None)


        if not return_dict:
            output = (logits,) + outputs[2:]
            # return ((loss,) + output) if loss is not None else output
            return {'loss': {'loss': loss}, 'logits': logits, 'hidden_attention': outputs[2:], 'z': torch.rand(2,3)}
