...
import global_var
from modeling_vaeroberta import RobertaForTokenClassification
from modeling_vaebert import BertForTokenClassification
...
MODEL_CLASSES = {
  ...
  "xlmr": (XLMRobertaConfig, RobertaForTokenClassification, XLMRobertaTokenizer),
}
...
def adapter_weights(args, model, i):
    ...
    for n, p in model.named_parameters():
        if 'encoder.layer.%r.output.adapter' % i in n:
            if 'weight' in n:
                params.append(p)
            elif 'bias' in n:
                params1.append(p)
...
def non_adapter_weights(args, model):
    params = []
    params1 = []
    for n, p in model.named_parameters():
        if 'output.adapter' not in n:
            if 'weight' in n:
                params.append(p)
            elif 'bias' in n:
                params1.append(p)
...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None):
  ...
  #optimizer_grouped_parameters = non_adapter_weights(args, model)
  ...
  for i in [int(x) for x in args.layer_sets.split(',')]:
      tmp_optimizer = AdamW(adapter_weights(args, model, i), lr=args.learning_rate * 50, eps=args.adam_epsilon)
      ae_optimizer.append(tmp_optimizer)
      ae_scheduler.append(get_linear_schedule_with_warmup(tmp_optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total))
  ...
  global_var.pretrain_ae = True
  for epoch in train_iterator:
    epoch = epoch + 1
    args.epoch = epoch
    ### AE START ###
    #if epoch >= int(args.num_train_epochs) - 40:
    #    print('### Last epoch training without AEs')
    #    global_var.last_epoch_training = True

    if epoch < 20:
        print('### epoch training freeze pretrained model')
        for name, param in model.named_parameters():
            # if 'encoder' in name and 'layer' in name and ('adapter' not in name and 'LayerNorm' not in name):
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      if (step + 1) % args.gradient_accumulation_steps == 0:
        ...
        optimizer.step()
        # update adapter loss only for pretraining stage
        if global_var.pretrain_ae:
          try:
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    features = []
    for lg in langs:
      data_file = os.path.join(args.data_dir, lg, "{}.{}".format(mode, args.model_name_or_path_short))
...
def main():
  ...
  args.model_name_or_path_short = args.model_name_or_path.rstrip('/').split('/')[-1]
  args.model_name_or_path_short = 'xlm-roberta-large'
  ...
  if args.do_predict and args.local_rank in [-1, 0]:
    ...
    with open(output_test_results_file, "a") as result_writer:
      for lang in args.predict_langs.split(','):
        print(os.path.join(args.data_dir, lang, 'test.{}'.format(args.model_name_or_path_short)))
        if not os.path.exists(os.path.join(args.data_dir, lang, 'test.{}'.format(args.model_name_or_path_short))):
        ...
        infile = os.path.join(args.data_dir, lang, "test.{}".format(args.model_name_or_path_short))
        idxfile = infile + '.idx'
  ...
  if args.do_predict_dev and args.local_rank in [-1, 0]:
    ...
    with open(output_test_results_file, "w") as result_writer:
      for lang in args.predict_langs.split(','):
        if not os.path.exists(os.path.join(args.data_dir, lang, 'dev.{}'.format(args.model_name_or_path_short))):
          logger.info("Language {} does not exist".format(lang))
          continue
        ...
        infile = os.path.join(args.data_dir, lang, "dev.{}".format(args.model_name_or_path_short))
        idxfile = infile + '.idx'
        save_predictions(args, predictions, output_test_predictions_file, infile, idxfile)
