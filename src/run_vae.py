...
from modeling_vaeroberta import RobertaForSequenceClassification
from modeling_vaebert import BertForSequenceClassification
# from modeling_vaedebertav2 import DebertaV2ForSequenceClassification
...
def adapter_weights(args, model, i):

    params = []
    params1 = []
    for n, p in model.named_parameters():
        if '%s.encoder.layer.%r.output.adapter'%(args.model_type, i) in n:
            if 'weight' in n:
                params.append(p)
            elif 'bias' in n:
                params1.append(p)
    ...
    return [
        ...
        {'params': params1, 'weight_decay': 0.0}
    ]


def adapter_encoder_weights(args, model, i):
    adp_num = len(args.vae_sets.split(','))

    params_name = []
    for j in range(0, adp_num):
        params_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.mlp.0.weight'%(args.model_type, i, j))
        params_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.mlp.2.weight'%(args.model_type, i, j))
        params_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.emb2mu.weight'%(args.model_type, i, j))
        params_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.emb2std.weight'%(args.model_type, i, j))

    params1_name = []
    for j in range(0, adp_num):
        params1_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.mlp.0.bias'%(args.model_type, i, j))
        params1_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.mlp.2.bias'%(args.model_type, i, j))
        params1_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.emb2mu.bias'%(args.model_type, i, j))
        params1_name.append('%s.encoder.layer.%r.output.adapter.aes.%r.emb2std.bias'%(args.model_type, i, j))

    params = [p for n, p in model.named_parameters() if n in params_name]
    params1 = [p for n, p in model.named_parameters() if n in params1_name]
...
def train(args, train_dataset, model, tokenizer):
    ...
    # optimizers for VAE
    vae_encoder_optimizer = []
    vae_encoder_scheduler = []
    
    for i in [int(x) for x in args.layer_sets.split(',')]:
        tmp_optimizer = AdamW(adapter_encoder_weights(args, model, i), lr=args.learning_rate, eps=args.adam_epsilon)
        vae_encoder_optimizer.append(tmp_optimizer)
        vae_encoder_scheduler.append(get_linear_schedule_with_warmup(tmp_optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total))
    ...
    for i in [int(x) for x in args.layer_sets.split(',')]:
        tmp_optimizer = AdamW(adapter_weights(args, model, i), lr=args.learning_rate, eps=args.adam_epsilon)
        vae_optimizer.append(tmp_optimizer)
        vae_scheduler.append(get_linear_schedule_with_warmup(tmp_optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total))
    ...
    global_var.last_epoch_training = False
    global_step = 0
    epochs_trained = 0
    ...
    for epoch in train_iterator:
        ...
        num_pre_training_epoch = 5
        if epoch < num_pre_training_epoch:					
            print('### First epoch training freeze pretrained model')
            ...
            print(epoch, '### epoch training all params')
            for name, param in model.named_parameters():
                if 'adapter' not in name:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # adapter loss
            vae_loss = []
            for i in range(len(global_var.kl_loss)):
                if global_var.kl_loss[i] is not None:
                    vae_loss.append(global_var.kl_loss[i])
            
            # recon loss
            recon_loss = []
            for i in range(len(global_var.recon_loss)):
                if global_var.recon_loss[i] is not None:
                    recon_loss.append(global_var.recon_loss[i])
                ...
                if True:
                    ...
                    if adp is not None:
                        for i in range(len(vae_loss)):
                            l = vae_loss[i]
                            #print(l)
                            vae_encoder_optimizer[i].zero_grad()
                            l.backward(retain_graph=True)
                            vae_encoder_optimizer[i].step()
                            vae_encoder_scheduler[i].step()

                        for i in range(len(recon_loss)):
                            l = recon_loss[i]
                            vae_optimizer[i].zero_grad()
                            l.backward(retain_graph=True)
                            vae_optimizer[i].step()
                            vae_scheduler[i].step()
        ...
        if args.evaluate_after_each_epoch:
            ...
            if results['acc_dev'] > best_dev:
                best_dev = results['acc_dev']
                best_result = results
            #save_model(args, global_step, model, optimizer, scheduler, tokenizer)
    with open(os.path.join(args.output_dir, "opt.txt"), 'a') as f:
...
def get_args():
    ...
    parser.add_argument("--adapter_loss_weight", default=0.01, type=float, help="Weight for adapter loss")
    parser.add_argument("--recon_loss_weight", default=0.001, type=float, help="Weight for recon loss")
    ...
    parser.add_argument("--num_sets", type=int, default=1, help="Number of vae sets")
    parser.add_argument("--vae_sets", type=str, default="6,8,11", help="Which layer to add VAEs")
    parser.add_argument("--layer_sets", type=str, default="128,256,512", help="VAEs dimension settings")
    parser.add_argument("--use_vae_in_test", action="store_true", help="Whether use VAE in test phase")
...
def main():
    ...
    # set params for VAE
    config.vae_sets = [int(x) for x in args.vae_sets.split(',')] * args.num_sets
    config.layer_sets = [int(x) for x in args.layer_sets.split(',')]
    config.use_vae_in_test = args.use_vae_in_test
    config.adapter_loss_weight = args.adapter_loss_weight
